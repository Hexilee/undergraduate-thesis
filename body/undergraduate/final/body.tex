...
之后，在\ref{sec:net}\nameref{sec:net}中将介绍HTTPS和WebSocket，这两种协议结合使用可以保证客户端和服务器之间的数据传输是实时且安全的。

最后，\ref{sec:programming-language}\nameref{sec:programming-language}中将简要介绍两种现代化的编程语言：Rust和TypeScript。
...
而HTTP/1.x是不支持双向流的，可选的升级方案有WebSocke和HTTP/2。

另外，基于TCP的HTTP协议是不安全的，为了保证软件数据传输的安全性，应当使用基于TLS协议的HTTPS。
