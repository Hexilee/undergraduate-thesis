...
\subsection{曲线重建模块}
\label{sec:reconst-mod}
图形学上常用的连续化算法（曲线）有贝塞尔（Bezier）曲线、B样条曲线、三次样条曲线等；
而统计学上常用线性插值、多项式插值等。图形学连续化的常见目的是得到光滑曲线，而统计学则追求最小回归方差。

对基于曲率的曲线重建而言，插值算法的选用约束了原始数据。
比如，采用B样条曲线插值则要求原始曲线的曲率变化是整体光滑的，否则会导致较大的插值误差。
多项式插值的拟合基准虽然是最小回归方差，但也较为光滑（$n$阶可导）。
线性插值对原始数据的约束更弱：它只要求每两个采样点之间曲率变化是单调的，而对数据整体无约束。
故本文采用其中较通用的线性插值法。
...
根据~\ref{sec:reconst-mod}小节，每两个采样点之间曲率变化应当单调，
故在二维标准余弦曲线的第一个周期内，间隔$\frac{\pi}{4}$取采样点以生成原始数据。
...
\subsubsection{拟合算法对比}
第~\ref{sec:reconst}节\nameref{sec:reconst}中主要提到了两种拟合算法，一种是经典Cartesian拟合算法，
另一种是基于Cartesian坐标系的改良算法，本小节将对比它们的实际准确度和性能。

图~\ref{fig:cos-diff-method}为插值步长$0.01$时，使用不同算法的余弦曲线重建结果分析。
其中图~\ref{cos-diff-method:elapsed}为重建性能对比图，使用两种算法各重建$1000$次，
横、纵轴分别为重建时间（单位/毫秒）和对应的样本数，红色方块、蓝色圆盘分别代表经典算法和改良算法；
图~\ref{cos-diff-method:error}展示了随横坐标$x$变化的绝对误差；
图~\ref{cos-diff-method:relative-error}展示了随横坐标$x$变化的相对误差。

可见经典算法和改良算法的重建误差完全一致，准确度没有任何差别。
但经典算法的运行时间极不稳定，最高的重建耗时接近7毫秒；而改良算法稳定维持在2毫秒以内。

\begin{figure}
\centering
\subfloat[重建性能]{\includegraphics{cos-diff-method-elapsed.pdf}\label{cos-diff-method:elapsed}}\\
\subfloat[绝对误差]{\includegraphics[scale=0.5]{cos-diff-method-error.pdf}\label{cos-diff-method:error}}
\subfloat[相对误差]{\includegraphics[scale=0.5]{cos-diff-method-relative-error.pdf}\label{cos-diff-method:relative-error}}\\
\caption{使用不同算法的余弦曲线重建}
\label{fig:cos-diff-method}
\end{figure}
\FloatBarrier
...
比较了$0.1$、$0.01$、$0.001$三种插值步长下的重建结果后可以认为$0.01$的插值步长更适于本文软件构建；
另外，比较了经典Cartesian拟合算法和改良算法后，可认为改良算法在没有准确度损失的情况下提升了性能。
...
HTTPS保证软件的安全性极佳；
在\nameref{sec:error-analyze}中对标准余弦曲线重建的相对误差在$0.03$以内，准确性良好；
