...
一种可视化软件的架构如图 ~\ref{fig:software}。
其工作流程可分为原始数据上推、数据预处理（连续化）、曲线坐标重建、重建数据下推和图形渲染五个步骤，
涉及的主要数据接口有原始数据、连续数据和重建数据。

\begin{figure}
\centering
\includegraphics[scale=0.3]{software-structure.png}
\caption{三维可视化软件架构图}
\label{fig:software} 
\end{figure}
...
故本文采用两个正交方向$a$、$b$的曲率数据为原始数据，如代码块~\ref{lst:raw-data}所示（其中采用的数据表达格式为JSON\cite{rfc7159}）。
...
\subsubsection{功能指标}
通用实时可视化软件的主要功能指标有通用性、安全性、准确性和实时性。

其中通用性指客户端是否通用，包括原始数据上推客户端和渲染客户端。原始数据上推客户端的通用性就是上推数据接口的通用性，
即FOSS的感测数据是否普遍能满足软件的原始数据接口。根据\nameref{sec:ss}，原始数据上推客户端的通用性优良。
而渲染客户端的通用性指客户端软件的运行是跨平台的、使用简便的。根据\ref{sec:render-choise}小节，
基于WebGL的渲染客户端通用性极佳。
...
\subsection{本章小结}
本章主要讨论了形变传感器的研究进展和渲染技术的选择，从而确定了软件的整体架构并提出了软件的功能指标，
但软件的具体构建还需要考虑以下三个问题：
