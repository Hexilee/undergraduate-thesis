...
本结主要介绍改良的重建算法和曲线重建可视化的实现步骤，具体的软件构建方案将在后面三章中介绍。
...
可视化的实现大致可分为三步：数据连续化、曲线坐标重建和图形渲染。
本小结将详细介绍这三个步骤，其中采用的数据表达格式为JSON\cite{rfc7159}。
...
并提出了一种改良的曲线重建算法和完整的三维可视化方案。这些理论和方案的具体实践将会在后面三章中介绍。